import { PrismaClient, AIAgentType } from '@prisma/client'
...
async function seedAIAgents() {
  try {
    logger.info('Starting AI Agents seeding...')
    ...
    for (const agentData of aiAgents) {
      ...
      if (existingAgent) {
        ...
        logger.info(`Updated AI Agent: ${agentData.name}`)
      } else {
        // Create new agent
        await prisma.aIAgent.create({
          data: agentData,
        })
        logger.info(`Created AI Agent: ${agentData.name}`)
      }
    }

    logger.info('✅ AI Agents seeding completed successfully')
  } catch (error) {
    logger.error('❌ AI Agents seeding failed:', error)
    throw error
  } finally {
